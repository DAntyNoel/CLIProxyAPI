...
func BuildConfigChangeDetails(oldCfg, newCfg *config.Config) []string {
	...
	if oldCfg.Codex.RepeatedToolLoopThreshold != newCfg.Codex.RepeatedToolLoopThreshold {
		changes = append(changes, fmt.Sprintf("codex.repeated-tool-loop-threshold: %d -> %d", oldCfg.Codex.RepeatedToolLoopThreshold, newCfg.Codex.RepeatedToolLoopThreshold))
	}
