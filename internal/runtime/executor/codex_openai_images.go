...
func (e *CodexExecutor) executeOpenAIImage(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	results, createdAt, usageRaw, firstMeta, errExtract := codexExtractImageResultsFromCollected(outputItemsByIndex, outputItemsFallback)
	if errExtract != nil {
		return resp, errExtract
	}
	if len(results) > 0 {
...
func (e *CodexExecutor) executeOpenAIImageStream(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (_ *cliproxyexecutor.StreamResult, err error) {
	...
	go func() {
		...
		results, _, usageRaw, _, errExtract := codexExtractImageResultsFromCollected(outputItemsByIndex, outputItemsFallback)
		if errExtract != nil {
			sendError(errExtract)
			return
		}
...
func codexExtractImageResultsFromCollected(itemsByIndex map[int64][]byte, fallback [][]byte) (results []codexImageCallResult, createdAt int64, usageRaw []byte, firstMeta codexImageCallResult, err error) {
	if len(itemsByIndex) == 0 && len(fallback) == 0 {
		return nil, 0, nil, codexImageCallResult{}, nil
	}
	completed := []byte(`{"type":"response.completed","response":{"output":[]}}`)
	return codexExtractImageResults(completed, itemsByIndex, fallback)
}
