...
func TestCodexExecutorExecuteOpenAIImageStreamUsesOutputItemDoneFallback(t *testing.T) {
	...
	executor := NewCodexExecutor(&config.Config{})
	result, err := executor.ExecuteStream(context.Background(), newCodexOpenAIImageTestAuth(server.URL), cliproxyexecutor.Request{
		Model:   "gpt-image-1",
		Payload: []byte(`{"model":"gpt-image-1","prompt":"draw a cat","stream":true}`),
	...
	var payload []byte
	for _, line := range bytes.Split(completed, []byte("\n")) {
